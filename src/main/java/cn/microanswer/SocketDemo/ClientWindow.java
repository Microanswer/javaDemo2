...
class ClientWindow extends JFrame {
    ...
    private void onUserOffLine(Room.Member member) {
        ...
        for (Map.Entry<String, Room> entry : entries) {
            ...
            if (value.getMembers().contains(member)) {
                // 下线的用户在此聊天室中，将聊天室关闭。
                int i1 = jTabbedPane.indexOfTab(value.getName());
                jTabbedPane.removeTabAt(i1);
                remove.add(entry.getKey()); // 记录要移除的数据。 （不能在foreach循环里面直接移除循环体数据）
            }
        }

        for (String s : remove) {
            talkingRooms.remove(s);
        }
    ...
    private void onUserRenamed(String fromClientId, String newName) {
        ...
        // 遍历所有的聊天室，将聊天室中的该用户也更名 TODO 名字更改处理。
        for (Map.Entry<String, Room> entry: talkingRooms.entrySet()) {
            Room.Member temp = new Room.Member(fromClientId, "");
            ArrayList<Room.Member> members = entry.getValue().getMembers();
            int i = members.indexOf(temp);
            if (i > -1) {
                temp = members.get(i);
    ...
    class MClientListener implements Client.ClientListener {
        ...
        @Override
        public void onNamed(Client client, String name, String id) {
            SwingUtilities.invokeLater(new Runnable() {
                @Override
                public void run() {
                    setLocalUserName(name);
        ...
        public void onError(Client client, Exception e) {
            SwingUtilities.invokeLater(new Runnable() {
                ...
                public void run() {
                    ...
                    JOptionPane.showMessageDialog(ClientWindow.this, "客户端错误：" + e.getMessage(), Constant.ERROR, JOptionPane.ERROR_MESSAGE);
                }
            });
        }
        ...
        public void onMsg(Client client, Msg msg) {
            ...
            if (0 < msgType && msgType <= 10) {
                ...
                if (msgType == MsgHead.TYPE_SYSTEM_REQUEST_ALL_USER) {
                    ...
                    // 解析返回数据
                    Room room = JSON.parseObject(text, Room.class);
                    SwingUtilities.invokeLater(new Runnable() {
                        @Override
                        public void run() {
                            onRoomCreated(room);
