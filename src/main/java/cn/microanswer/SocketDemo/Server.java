...
public class Server extends Thread {
    ...
    void onClientConnted(Client client) {
        ...
        String name = ("用户-" + UUID.randomUUID().toString().substring(0, 4));
        String id = Utils.md5(String.valueOf(System.nanoTime()));
    ...
    private Client.ClientListener clientListener = new Client.ClientListener() {
        @Override
        public void onConneted(final Client client, String host, int port) {
            SwingUtilities.invokeLater(new Runnable() {
                @Override
                public void run() {
        ...
        @Override
        public void onNamed(final Client client, final String clientName, final String clientId) {
            // 告诉所有的在线人员，有人改名字了。
            ...
            for (Map.Entry<String, Room> e : rooms.entrySet()) {
                e.getValue().memberReNamed(clientId, clientName);
            }
